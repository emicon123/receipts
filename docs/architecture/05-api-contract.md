...
| GET | `/api/receipts` | PWA | Paginated list, filterable by `year`, `month`, `status`. Default sort `capturedAt` desc. |
| GET | `/api/receipts/pending` | **classify-receipts.sh** | Lean `{id}`-only list of everything `PENDING`. Pure read — never mutates status (see `03-receipt-lifecycle.md`). Unpaginated by design. |
| GET | `/api/receipts/{id}` | PWA | Full detail incl. `imageUrl` + line items. |
| GET | `/api/receipts/{id}/image` | PWA, **classify-receipts.sh** | Raw image bytes. 404 for a `MANUAL` receipt (no image) or unknown id. |
| POST | `/api/receipts/classification-batch` | **classify-receipts.sh** | Body is Claude's raw `{items, failures}` output, forwarded unchanged. Idempotent per receipt; replaces only uncorrected line items; tolerant of an unknown `receiptId` or an out-of-enum `category` per entry (routed to `FAILED`/`skipped`, never a whole-request 400) — see below. |
...
| POST | `/api/receipts/{id}/reprocess` | PWA | Resets `FAILED` → `PENDING` freely; `PROCESSED`/`PROCESSING` → `PENDING` requires `force: true`. Pure status reset — no classification logic here. |
| DELETE | `/api/receipts/{id}` | PWA | Removes the row (cascade) and the image file. |
